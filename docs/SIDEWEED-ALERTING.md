...
| Alert delivery | **Отсутствует** — только `docker logs` / journal |

**Gap ТЗ 6.4:** logging ✓, alerting ✗.
...
## 4. Proposed metrics (Prometheus)

Префикс: `sideweed_`. Exporter: **встроенный** `/metrics` на write sideweed (proposal).

| Metric | Type | Labels | Описание | Cardinality notes |
|--------|------|--------|----------|-------------------|
| `sideweed_write_health_status` | **Gauge** | `instance` | 1=healthy, 0=degraded | Low — 1 per sideweed write instance |
| `sideweed_put_blocked_total` | **Counter** | `reason`, `method` | Заблокированные мутирующие запросы | Low — ~5 reasons × 3 methods |
| `sideweed_write_degraded_total` | **Counter** | `reason` | Переходы в degraded | Low — reason enum |
| `sideweed_write_recovered_total` | **Counter** | — | Переходы в recovered | Very low |
| `sideweed_backend_up` | **Gauge** | `backend`, `url_host` | 1=UP per S3 backend | **Medium** — limit to backend id, not full URL path |
| `sideweed_health_probe_duration_seconds` | **Histogram** | `probe` | Latency probe (s3, filer, master, assign) | Low — 4 probes |
| `sideweed_health_probe_success` | **Gauge** | `probe` | Last probe 1/0 | Low |
| `sideweed_put_block_duration_seconds` | **Histogram** | — | Time to return 503 (fail-fast) | Low — should be &lt;1s |
| `sideweed_http_requests_total` | **Counter** | `method`, `code`, `blocked` | Optional generic request counter | Medium — avoid `path` label |
