...
Отдельного `get_snapshot.sh`, range-query по времени и записи metadata через filer **нет**.
...
| Tests | archive PUT/GET, snapshot PUT/GET, range query smoke |
...
- Bucket `vab`, production pipeline §5.2.
...
- `get_snapshot.sh`, range queries, benchmarks;
