...
**Runtime:** `schema.cql` без изменений; `schema-v2.cql` — draft, не в compose.  
**Production (audit):** `seaweedfs.filemeta` с TWCS — **не** stand `video_archive.fragments`. См. [PRODUCTION-CONFIG-AUDIT.md](PRODUCTION-CONFIG-AUDIT.md).

---

## 1. Текущее состояние

### 1.0 Production Cassandra (из audit, read-only)

| Параметр | Production (`seaweedfs.filemeta`) | Stand (`video_archive.fragments`) |
|----------|-----------------------------------|-----------------------------------|
| Keyspace | `seaweedfs` | `video_archive` |
| Table | `filemeta` (filer blob index) | `fragments` (test client index) |
| PK | `(directory, name)` | `(camera_id, fragment_id)` |
| Compaction | **TWCS**, window 6h | default STCS |
| RF | 3 | 1 |
| Writer | SeaweedFS filer | stand `fragment` CLI |

Application metadata (teye keyspace `teye`) — **отдельный кластер**, DDL в архиве **не получен**.
...
### 1.3 Buckets (stand vs production)

| Назначение | ТЗ | Production (audit) | Stand |
|------------|-----|-------------------|-------|
| Video archive (blob) | `vab` | **`vab`** (streamserver storage) | `video-fragments` |
| Camera snapshots (blob) | `csb` | **`vab`** (write); **`csb`** read path ready | `csb` (`put_snapshot.sh`) |
| Event snapshots | `esb` | **`esb`** | не моделируется на stand |

См. [PRODUCTION-CONFIG-AUDIT.md](PRODUCTION-CONFIG-AUDIT.md) §4.

### 1.4 Отличие от production (важно)

По [TZ-DEVIATIONS.md](TZ-DEVIATIONS.md):

- В production метаданные blob index пишет **SeaweedFS filer** в `seaweedfs.filemeta` (TWCS).
- Application metadata — keyspace **`teye`** на отдельном DB-кластере (DDL не в архиве).
- Production configs **получены** (audit); **изменения не внесены**.
