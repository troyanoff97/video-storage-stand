...
| Пункт | Статус | Что сделано | Подтверждение | Что осталось | Блокер |
|-------|--------|-------------|---------------|--------------|--------|
| **6.1 Health checks** | **Partial** | Write probes (S3, filer, master, assign); **`GET /v1/write-health`** JSON + per-probe visibility | [sideweed-health.md](sideweed-health.md), `make test-sideweed` | Direct per-volume probes; multi-master list; prod multi-S3-GW | — |
