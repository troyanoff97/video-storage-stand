...
## Direct volume-node visibility (design, §6.1 gap)

ТЗ требует visibility по **master + volume nodes**. Текущая production-safe модель:

| Уровень | Роль | Блокирует PUT? |
|---------|------|----------------|
| Aggregate assign | `assign` probe → master `/dir/assign` | **Да**, если нет writable volume |
| Per-component | s3, filer, master status | **Да** при down |
| Per-volume | `volumeN /status` или `/healthz` | **Нет** (visibility only) |

**Почему volume probes не в write gate:** при `replication=000` и N>1 volume один узел может быть down, а `assign` всё ещё **200** → PUT должен проходить. Добавление `--write-health-check volume1=http://volume1:8080/status` в write gate **сломает** сценарий *single volume down* (`make test-sideweed` ожидает PUT **200**).

Sideweed **уже поддерживает** произвольные `--write-health-check=name=url[|code]` (см. `sideweed/main.go`), но для volume — только как **отдельный non-gating** канал (будущее):

- отдельный флаг `--volume-health-check` (visibility, metrics, warning alerts);
- или debug compose profile `sideweed-volumes` без write gate blocking;
- vmalert: warning на `probe=volume1` failed при `write_health_status==1`.

**Рекомендация для stand:** не менять `docker-compose.yml` write sideweed; aggregate assign остаётся source of truth для PUT blocking. См. [TZ-ACCEPTANCE-MATRIX.md](TZ-ACCEPTANCE-MATRIX.md) §6.1.
