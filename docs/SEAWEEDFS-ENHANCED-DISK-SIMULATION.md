...
## 12. Ручная верификация (локально)

**Дата:** 2026-06-25. **Commits:** `4484d8c`, `079a9f8`. **Среда:** `/home/cerf/Desktop/work2`, `SUDO_ASKPASS`, `CONFIRM_DISK_SIM=1`.

| Сценарий | Скрипт | Результат |
|----------|--------|-----------|
| Setup loopback ext4 | `setup_loopback_dirs.sh` | **PASS** |
| Disk full | `test_disk_full.sh` | **PASS** (100% df, block write fails) |
| Recovery | `recover_mounts.sh` | **PASS** |
| Read-only remount | `test_readonly_mount.sh` | **PASS** (`ro,relatime`) |
| Mount unavailable | `test_mount_unavailable.sh` | **PASS** |
